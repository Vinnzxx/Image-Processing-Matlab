...
% ciri_sunflowers(i,12) = L;
% ciri_sunflowers(i,13) = A;
% ciri_sunflowers(i,14) = B;
...
%membaca folder/file
folder = 'data latih/daisy';
file = dir(fullfile(folder, '*.jpg'));
jumlah = numel(file);

%inisialisasi variabel untuk sebagai dataset
%3 akan diisi dengan nilai rgb
ciri_daisy = zeros(jumlah, 11);
%cell krn akan diisi string sdngkn zeros hanya bisa diisi angka
...
training = [ciri_sunflowers;ciri_daisy];
target = [target_sunflowers;target_daisy];

%training dataset menggunakan algoritma knn
Mdl11 = fitcknn(training,target,"NumNeighbors",11,Distance="minkowski",CategoricalPredictors="all")
%melakukan predict
hasil = predict(Mdl11,training);
...
Akurasi_Train_KNN_2_Classification = benar/data*100
